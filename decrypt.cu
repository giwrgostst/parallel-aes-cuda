/******************************************************************************
 * AES-128 DECRYPTION USING CUDA
 * 
 * Single-block demonstration:
 * - Decrypts one 128-bit block (ciphertext) with a 128-bit key.
 ******************************************************************************/

#include <stdio.h>
#include <stdint.h>
#include <iostream>

// AES block size in bytes
#define AES_BLOCK_SIZE 16

// -----------------------------------------------------------------------------
// Device-side constants
// -----------------------------------------------------------------------------
__constant__ uint8_t d_sbox[256] = {
    
};

__constant__ uint8_t d_inv_sbox[256] = {
    
};

__constant__ uint8_t d_Rcon[11] = {
    0x00,
    0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80,
    0x1B, 0x36
};

// -----------------------------------------------------------------------------
// Device-side utility functions
// -----------------------------------------------------------------------------

// Galois field multiplication (same as in encryption)
__device__ uint8_t galois_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) {
            a ^= 0x1B;  // x^8 + x^4 + x^3 + x + 1
        }
        b >>= 1;
    }
    return p;
}

__device__ void add_round_key(uint8_t state[4][4], uint8_t round_key[4][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] ^= round_key[i][j];
        }
    }
}

__device__ void inv_sub_bytes(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = d_inv_sbox[state[i][j]];
        }
    }
}

// Inverse ShiftRows (reverse of encryption's shift_rows)
__device__ void inv_shift_rows(uint8_t state[4][4]) {
    uint8_t temp;

    // Row 1: shift right by 1
    temp          = state[1][3];
    state[1][3]   = state[1][2];
    state[1][2]   = state[1][1];
    state[1][1]   = state[1][0];
    state[1][0]   = temp;

    // Row 2: shift right by 2
    temp          = state[2][0];
    state[2][0]   = state[2][2];
    state[2][2]   = temp;
    temp          = state[2][1];
    state[2][1]   = state[2][3];
    state[2][3]   = temp;

    // Row 3: shift right by 3 (equivalent to shift left by 1)
    temp          = state[3][0];
    state[3][0]   = state[3][1];
    state[3][1]   = state[3][2];
    state[3][2]   = state[3][3];
    state[3][3]   = temp;
}

__device__ void inv_mix_columns(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++) {
        // For InvMixColumns, multiply columns by the inverse matrix {0x0e,0x0b,0x0d,0x09}
        uint8_t a0 = state[0][i];
        uint8_t a1 = state[1][i];
        uint8_t a2 = state[2][i];
        uint8_t a3 = state[3][i];

        state[0][i] = galois_mul(a0,0x0e) ^ galois_mul(a1,0x0b) ^ galois_mul(a2,0x0d) ^ galois_mul(a3,0x09);
        state[1][i] = galois_mul(a0,0x09) ^ galois_mul(a1,0x0e) ^ galois_mul(a2,0x0b) ^ galois_mul(a3,0x0d);
        state[2][i] = galois_mul(a0,0x0d) ^ galois_mul(a1,0x09) ^ galois_mul(a2,0x0e) ^ galois_mul(a3,0x0b);
        state[3][i] = galois_mul(a0,0x0b) ^ galois_mul(a1,0x0d) ^ galois_mul(a2,0x09) ^ galois_mul(a3,0x0e);
    }
}

// Key expansion (same as encryption)
__device__ void key_expansion(uint8_t key[4][4], uint8_t round_keys[11][4][4]) {
    uint32_t w[44];

    // Copy the original key into the first 4 words
    for (int i = 0; i < 4; i++) {
        w[i] = ((uint32_t)key[0][i] << 24) |
               ((uint32_t)key[1][i] << 16) |
               ((uint32_t)key[2][i] << 8)  |
               (uint32_t)key[3][i];
    }

    // Expand the key
    for (int i = 4; i < 44; i++) {
        uint32_t temp = w[i - 1];
        if ((i % 4) == 0) {
            // RotWord
            temp = (temp << 8) | (temp >> 24);

            // SubWord using forward S-Box
            temp = ((uint32_t)d_sbox[(temp >> 24) & 0xFF] << 24) |
                   ((uint32_t)d_sbox[(temp >> 16) & 0xFF] << 16) |
                   ((uint32_t)d_sbox[(temp >>  8) & 0xFF] << 8)  |
                   (uint32_t)d_sbox[temp & 0xFF];

            // Rcon
            temp ^= ((uint32_t)d_Rcon[i / 4] << 24);
        }
        w[i] = w[i - 4] ^ temp;
    }

    // Store expanded key into round_keys
    for (int round = 0; round < 11; round++) {
        for (int i = 0; i < 4; i++) {
            uint32_t word = w[round * 4 + i];
            round_keys[round][0][i] = (word >> 24) & 0xFF;
            round_keys[round][1][i] = (word >> 16) & 0xFF;
            round_keys[round][2][i] = (word >>  8) & 0xFF;
            round_keys[round][3][i] =  word        & 0xFF;
        }
    }
}

__device__ void aes_decrypt_device(const uint32_t input[4], const uint32_t key[4], uint32_t output[4]) {
    // Convert input into state
    uint8_t state[4][4];
    for (int i = 0; i < 4; i++) {
        state[0][i] = (input[i] >> 24) & 0xFF;
        state[1][i] = (input[i] >> 16) & 0xFF;
        state[2][i] = (input[i] >>  8) & 0xFF;
        state[3][i] =  input[i]        & 0xFF;
    }

    // Convert key into 4x4
    uint8_t key_state[4][4];
    for (int i = 0; i < 4; i++) {
        key_state[0][i] = (key[i] >> 24) & 0xFF;
        key_state[1][i] = (key[i] >> 16) & 0xFF;
        key_state[2][i] = (key[i] >>  8) & 0xFF;
        key_state[3][i] =  key[i]        & 0xFF;
    }

    // Generate round keys
    uint8_t round_keys[11][4][4];
    key_expansion(key_state, round_keys);

    // Initial AddRoundKey
    add_round_key(state, round_keys[10]);

    // Rounds 1 to 9 (in reverse order)
    for (int round = 9; round >= 1; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, round_keys[round]);
        inv_mix_columns(state);
    }

    // Final Round
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, round_keys[0]);

    // Copy state back to output (plaintext)
    for (int i = 0; i < 4; i++) {
        output[i] = (state[0][i] << 24) |
                    (state[1][i] << 16) |
                    (state[2][i] <<  8) |
                     state[3][i];
    }
}

// -----------------------------------------------------------------------------
// CUDA Kernel
// -----------------------------------------------------------------------------
__global__ void aes_decrypt_kernel(const uint32_t *d_input,
                                   const uint32_t *d_key,
                                   uint32_t *d_output)
{
    // Single block, single thread approach for demonstration
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        aes_decrypt_device(d_input, d_key, d_output);
    }
}

int main() {
    // Example data
    uint32_t h_original_plaintext[4] = {
        0x3243f6a8,
        0x885a308d,
        0x313198a2,
        0xe0370734
    };

    // 128-bit key
    uint32_t h_key[4] = {
        0x2b7e1516,
        0x28aed2a6,
        0xabf71588,
        0x09cf4f3c
    };

    uint32_t h_ciphertext[4] = {
        0x3925841d,
        0x02dc09fb,
        0xdc118597,
        0x196a0b32
    };

    // Output buffer for decrypted plaintext
    uint32_t h_decrypted[4] = {0,0,0,0};

    // Allocate device memory
    uint32_t *d_ciphertext, *d_key, *d_decrypted;
    cudaMalloc((void**)&d_ciphertext, 4 * sizeof(uint32_t));
    cudaMalloc((void**)&d_key,        4 * sizeof(uint32_t));
    cudaMalloc((void**)&d_decrypted,  4 * sizeof(uint32_t));

    // Copy ciphertext and key to device
    cudaMemcpy(d_ciphertext, h_ciphertext, 4 * sizeof(uint32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_key,        h_key,        4 * sizeof(uint32_t), cudaMemcpyHostToDevice);

    // Launch kernel (1 block, 1 thread)
    aes_decrypt_kernel<<<1, 1>>>(d_ciphertext, d_key, d_decrypted);
    cudaDeviceSynchronize();

    // Copy decrypted plaintext back to host
    cudaMemcpy(h_decrypted, d_decrypted, 4 * sizeof(uint32_t), cudaMemcpyDeviceToHost);

    // Free device memory
    cudaFree(d_ciphertext);
    cudaFree(d_key);
    cudaFree(d_decrypted);

    // Print the decrypted plaintext
    std::cout << "Decrypted plaintext: ";
    for (int i = 0; i < 4; i++) {
        printf("%08x ", h_decrypted[i]);
    }
    std::cout << std::endl;

    // Check
    bool match = true;
    for (int i = 0; i < 4; i++) {
        if (h_decrypted[i] != h_original_plaintext[i]) {
            match = false;
            break;
        }
    }
    if (match) {
        std::cout << "Test Passed: The decrypted plaintext matches the original value!" << std::endl;
    } else {
        std::cout << "Test Failed: The decrypted plaintext does NOT match the original value." << std::endl;
    }

    return 0;
}
